...
def non_max_suppression(
    ...
    Returns:
        ...
        if return_full:
         list of detections, on (n, 6 + nc) tensor per image [xyxy, cls, bbx_conf, cls_conf (for all classes)]
    """
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # 2. Build both tensors explicitly
        if multi_label:
            i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)
            xfull = torch.cat((box[i], j[:, None].float(), x_raw[i, 4:]), 1)

        else:
            conf, j = x[:, 5:].max(1, keepdim=True)
            mask = conf.view(-1) > conf_thres
            x = torch.cat((box, conf, j.float()), 1)[mask]
            xfull = torch.cat((box, j.float(), x_raw[:, 4:]), 1)[mask]

        # 3. Apply all subsequent filters to both (example: classes)
        if classes is not None:
            mask = (x[:, 5:6] == torch.tensor(classes, device=x.device)).any(1)
            x, xfull = x[mask], xfull[mask]

        # 4. Apply sorting to both
        n = x.shape[0]
        if n > max_nms:
            idx = x[:, 4].argsort(descending=True)[:max_nms]
            x, xfull = x[idx], xfull[idx]
        ...
        output[xi] = xfull[i] if return_full else x[i]
