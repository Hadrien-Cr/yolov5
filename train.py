...
def train(
    ...
    if RANK in [-1, 0]:
        loggers = Loggers(
            save_dir, weights, opt, hyp, data_dict, LOGGER
        ).start()  # loggers dict
        if loggers.wandb and resume:
            weights, epochs, hyp, data_dict = (
                opt.weights,
                opt.epochs,
                opt.hyp,
                loggers.wandb.data_dict,
            )
    ...
    for epoch in range(
        ...
        if RANK in [-1, 0]:
            pbar = tqdm(pbar, total=nb, mininterval=1.0)  # progress bar
        optimizer.zero_grad()
        for i, (
            imgs,
            targets,
...
def main(opt):
    ...
    # Evolve hyperparameters (optional)
        ...
        if opt.bucket:
            os.system(
                f"gsutil cp gs://{opt.bucket}/evolve.txt ."
