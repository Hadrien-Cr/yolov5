...
def process_batch(predictions, labels, iouv):
    ...
    correct = torch.zeros(
        predictions.shape[0], len(iouv), dtype=torch.bool, device=iouv.device
    )
    ...
    for cls in torch.unique(tcls):
        ...
        if pi.shape[0]:  # find detections
            ious, i = box_iou(predictions[pi, 0:4], labels[ti, 1:5]).max(
                1
            )  # best ious, indices
            detected_set = set()
...
def run(
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s, mininterval=10.0)):
        t_ = time_sync()
        ...
        out = non_max_suppression(
            out, conf_thres, iou_thres, labels=lb, multi_label=False, agnostic=True
        )
        t2 += time_sync() - t
